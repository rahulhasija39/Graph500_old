...
void setup_globals() {
 rank = MYTHRED; //MPI_Comm_rank(MPI_COMM_WORLD, &rank);
 size = THREADS;// MPI_Comm_size(MPI_COMM_WORLD, &size);
...
void* xUPC_Alloc_mem(size_t nbytes) {
  void* p;
  MPI_Alloc_mem(nbytes, MPI_INFO_NULL, &p);   ///What do I do here because I think I would need something for realloc too yes?
  if (!p) {
