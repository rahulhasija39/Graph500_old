...
int validate_bfs_result(const csr_graph* const g, const int64_t root, const int64_t* const pred, const int64_t nvisited) {
  ...
  shared int *any_range_errors = (shared int*) xUPC_Alloc_mem( sizeof( int));
	any_range_errors = 0;
  size_t i;
  ...
  upc_all_reduce( (shared int*) any_range_errors, (shared int*) any_range_errors, (upc_op_t) UPC_LOR, THREADS, 1, NULL, sync_mode);

  /* Check that nvisited is correct. */
  //int64_t nvisited_actual = 0;
  shared int64_t *nvisited_actual = xUPC_Alloc_mem( sizeof( int64_t));
	nvisited_actual = 0;	
  ...
  upc_all_reduceI( (shared int64_t*) nvisited_actual, (shared int64_t*) nvisited_actual, (upc_op_t) UPC_SUM, THREADS, 1, NULL, sync_mode);
  ...
  if (!any_range_errors) { /* Other parts of validation assume in-range values */
    ...
    { /* Scope some code that has a lot of temporary variables. */
      ...
      shared int * any_changed = xUPC_Alloc_mem( sizeof( int));
      while (1) {
        //int any_changed = 0;
				any_change = 0;
        ...
        upc_all_reduceI( (shared int*) any_changed, (shared int*) any_changed, (upc_op_t) MPI_LOR, THREADS, 1, NULL, sync_mode);
        if (!any_changed) break;
      }
			upc_free(any_changed);
     ...
     * one. */
    {
      ...
      shared int64_t *maxlocaledges = xUPC_Alloc_mem( sizeof( int64_t));
			//maxlocaledges = nlocaledges; 
      maxlocaledges = (int64_t) nlocaledges;
      //MPI_Allreduce(MPI_IN_PLACE, &maxlocaledges, 1, INT64_T_MPI_TYPE, MPI_MAX, MPI_COMM_WORLD);
      upc_all_reduceI( (shared int64_t*)maxlocaledges, shared int64_t*)maxlocaledges, (upc_op_t) UPC_MAX, THREADS, 1, NULL, sync_mode);      
  ...
  shared int *validation_passed_upc = (shared int*) xUPC_Alloc_mem( sizeof( int));
	validation_passed_upc = (int) validation_passed; 
	upc_all_reduceI( (shared int*) validation_passed, (shared int*) validation_passed, (upc_op_t) UPC_LAND, THREADS, 1, NULL, sync_mode);
	validation_passed = (int) valication_passed_upc;
