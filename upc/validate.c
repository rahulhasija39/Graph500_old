...
/*	shared int * any_range_errors = (shared int*) xUPC_Alloc_mem( sizeof(int));
  ...
  upc_all_reduceI( (shared int*) any_range_errors, (shared int*) any_range_errors, UPC_LOGOR, THREADS, 1, NULL, sync_mode);

  /* Check that nvisited is correct. */
  //int64_t nvisited_actual = 0;
 /* shared int64_t *nvisited_actual = (shared int64_t*) xUPC_Alloc_mem( sizeof(int64_t));
	nvisited_actual = 0;
	for (i = 0; i < nlocalverts; ++i) {
    if (pred[i] != -1) ++nvisited_actual;
  }
  //MPI_Allreduce(MPI_IN_PLACE, &nvisited_actual, 1, INT64_T_MPI_TYPE, MPI_SUM, MPI_COMM_WORLD);
  upc_all_reduceI( (shared int64_t*) nvisited_actual, (shared int64_t*) nvisited_actua, UPC_ADD, THREADS, 1, NULL, sync_mode);
