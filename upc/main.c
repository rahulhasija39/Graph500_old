...
upc_flag_t sync_mode = UPC_IN_MYSYNC | UPC_OUT_MYSYNC;
  ...
  if (argc <= 1 || argc >= 4 || SCALE == 0 || edgefactor == 0) {
    ...
    upc_global_exit(1); //MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  double make_graph_start = bupc_ticks_now();//MPI_Wtime();
  make_graph(SCALE, (int64_t)(edgefactor * pow(2., SCALE)), seed1, seed2, initiator,
             &nedges, &edges);
  double make_graph_stop = bupc_ticks_now();//MPI_Wtime();
   ...
   * distribution. */
  double data_struct_start = bupc_tick_now(); //MPI_Wtime();
  convert_graph_to_csr(nedges, edges, &g);
  double data_struct_stop = bupc_ticks_now();
  ...
  for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
    ...
    /* Do the actual BFS. */
    double bfs_start = bupc_ticks_now();//MPI_Wtime();
    int64_t nvisited = 0;
    run_upc_bfs(&g, root, &pred[0], &nvisited);
    double bfs_stop = bupc_ticks_now();//MPI_Wtime();
    bfs_times[bfs_root_idx] = bfs_stop - bfs_start;

    /* Validate result. */
    if (rank == 0) fprintf(stderr, "Validating BFS %d\n", bfs_root_idx);

    double validate_start = bupc_ticks_now();//MPI_Wtime();
    int validation_passed_one = validate_bfs_result(&g, root, pred, nvisited);///////////////
    double validate_stop = bupc_ticks_now(); // MPI_Wtime();
